...
class Helpers {
   public:
    ...
    static char keywordToChar(Keywords keyword) {
        switch(keyword) {
    ...
    static bool isNumericKeyword(Keywords aKeyword) {
        static Keywords theTypes[] = {Keywords::decimal_kw, Keywords::double_kw,
                                      Keywords::float_kw, Keywords::integer_kw};
        for(auto k : theTypes) {
    ...
    void getRidOfWarnings() {
        Keywords kw = gJoinTypes[0];
        if(kw == Keywords::insert_kw) {
            std::cerr << "this should never be printed\n";
        }
    }
};
